...
while(var_sites > 5200 || var_sites < 4850 ){
  sim_uch <- simulate.uncor.lnorm(tree_temp, params = list(mean.log = log(uch_rate), sd.log = 0.1))
  phylogram_temp <- sim_uch$phylogram
...
if(var_sites < 1200 || var_sites > 850){
  write.dna(seq_data, file = gsub('tre', 'fasta', paste0('uch_', tree_name)), format = 'fasta', nbcol = -1, colsep = '')
}