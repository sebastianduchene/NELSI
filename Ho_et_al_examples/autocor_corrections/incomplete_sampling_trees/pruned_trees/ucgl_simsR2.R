...
The original simulations are:
    ...
    - UCG-L # shape = alpha, rate = beta
      alpha = 10, beta = 10^5 (2000)
      alpha = 10, beta = 10000
    - UCG-H
      alpha = 1, beta = 10^4 (200)
    - ACLN-L
      rate = 0.005, v = 0.3
    - ACLN-M
      rate = 0.005, v = 0.2
...
print("Running ucgl simulations")
...
var_sites <- 0
i <- 1
while(var_sites > 5200 || var_sites < 4850 ){
  sim_ucgl <- simulate.uncor.gamma(tree_temp, params = list(shape = ucgl_alpha, rate = ucgl_beta))
  phylogram_temp <- sim_ucgl$phylogram
...
if(var_sites < 1200 || var_sites > 850){
  write.dna(seq_data, file = gsub('tre', 'fasta', paste0('ucgl_', tree_name)), format = 'fasta', nbcol = -1, colsep = '')
}