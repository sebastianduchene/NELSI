...
while(var_sites > 1200 || var_sites < 850 ){
  sim_acm <- simulate.autocor.kishino(tree_temp, params = list(initial.rate = acm.rate, v = acm.v))
  phylogram_temp <- sim_acm$phylogram
...
if(var_sites < 1200 || var_sites > 850){
  write.dna(seq_data, file = gsub('tre', 'fasta', paste0('acm_', tree_name)), format = 'fasta', nbcol = -1, colsep = '')
}